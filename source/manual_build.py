...
def upload_objects(s3, bucket_name, s3_prefix, directory_name):
    try:
        ...
        for path, subdirs, files in os.walk(directory_name):
            ...
            for file in files:
                print("%s[+] Uploading %s to s3://%s/%s%s%s" % (
                fg('green'), os.path.join(path, file), bucket_name, s3_prefix, directory + '/' + file, attr('reset')))
...
if __name__ == "__main__":
    ...
    targets = ['scripts', 'templates', 'README.txt', 'scale-out-computing-on-aws.template',
    ...
    # Replace Placeholder
    for line in fileinput.input([build_folder + '/scale-out-computing-on-aws.template',
                                 build_folder + '/install-with-existing-resources.template'], inplace=True):
        print(line.replace('%%BUCKET_NAME%%', 'your-s3-bucket-name-here').replace('%%SOLUTION_NAME%%/%%VERSION%%',
                                                                                  'your-s3-folder-name-here').replace(
            '\n', ''))
    ...
    if s3_bucket_exists:
        ...
        print(
            "%s==> https://console.aws.amazon.com/cloudformation/home?region=%s#/stacks/create/review?&templateURL=%s&param_S3InstallBucket=%s&param_S3InstallFolder=%s%s" % (
            fg('light_blue'), region, template_url, bucket, output_prefix, attr('reset')))
        ...
        print(
            "2: Drag & Drop " + build_path + "/" + build_folder + " to your S3 bucket (eg: 'mysocacluster/" + build_folder + ")")
