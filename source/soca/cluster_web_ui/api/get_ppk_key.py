...
def f():
    ...
    generate_ppk = ['unix/puttygen', user_private_key_path, '-o', parameters.get_parameter('ssh', 'private_key_location')+'/' + username+'_hpc_privatekey.ppk']
    subprocess.call(generate_ppk)
    return send_file(parameters.get_parameter('ssh', 'private_key_location')+'/'+username+'_hpc_privatekey.ppk',
                     as_attachment=True,
                     attachment_filename=username+'_hpc_privatekey.ppk')
